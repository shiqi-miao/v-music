...
<template>
    <div class="app-container">
        ...
        v-loading.body="listLoading" element-loading-text="拼命加载中">
            ...
            <el-table-column prop="thresholdValue" label="采购数量（采购规格）" style="width: 18%" align="center">
                <template slot-scope="scope">
                    <el-input-number v-model="scope.row.recommendNum" :min="1" :max="scope.row.stockNum" size="mini" @change="changeNum(scope.row)"></el-input-number><span>{{scope.row.skuModel[0].pcsNum}}件/{{scope.row.skuModel[0].modelTypeName}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="thresholdValue" label="单价" style="width: 18%" align="center">
                <template slot-scope="scope">
                    <span>￥{{scope.row.skuModel[0].modelPrice}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="thresholdValue" label="预算" style="width: 18%" align="center">
                <template slot-scope="scope">
                    <span>￥{{scope.row.recommendNum*scope.row.skuModel[0].pcsNum*scope.row.skuModel[0].modelPrice}}</span>
                </template>
            </el-table-column>
        ...
        <div class="left"><el-button icon="el-icon-plus" round @click="addSKU()" >新增需采购SKU</el-button></div>
        <div class="center">
            <el-button type="primary" round @click="submit()">提交采购</el-button></div>
...
export default {
    ...
    methods:{
        ...
        getPurchaseCode(){
            this.api({
                url: "/ware/createPurchaseCode",
                method: "get",
            }).then(data => {
                this.purchaseCode = data
                // this.getSKUList({})
            })
        },
        changeNum(row){
        ...
        addSKUData(){
            ...
            this.api({
                ...
                //     if(this.skuTableData[i].skuCode==this.skuCode){
                //         this.skuTableData[i].recommendNum = 0
                //         lists.push(this.skuTableData[i])
                //         break;
                //     }
                // }
                // this.tableData = lists
