...
<template>
    <div class="app-container" id="commodity">
        <div style="background:#fff;min-height:85vh;position:relative;border:1px solid rgba(221,223,229,1);padding-top:10px">
        <div class="flex-center-Y" style="position:absolute;top:-36px;left:0">
            <div :class="activeTab=='WEB'?'tab tab1 active':'tab tab1'" @click="activeTab='WEB';getList()"><div class="dot" v-if="noticeNumWeb>0"></div>PC端</div>
            <div :class="activeTab=='WEB'?'tab':'tab active'" @click="activeTab='PHONE';getList()"><div class="dot" v-if="noticeNumPhone>0"></div>移动端</div>
        </div>
        <div style="padding:30px;padding-top: 0;">
        <div class="table">
            <div v-for="(item,i) in tableData" :key="i">
                <div class="line flex-center-Y" @click="changeStatus(item,i)">
                    <div class="td No">
                        <div class="dot" v-if="item.noticeStatus=='N'"></div>
                        {{item.noticeName}}
                    </div>
                    <div class="td time">
                        {{item.createTime}}
                    </div>
                    <div class="td content limit">
                        <span v-if="item.noticeNew">新增：{{item.noticeNew}}</span>
                        <span v-if="item.noticeRepair">修复：{{item.noticeRepair}}</span>
                    </div>
                    <div class="td more">
                        <i :class="item.active?'el-icon-caret-top':'el-icon-caret-bottom'"></i>
                    </div>
                </div>
                <div :class="item.active?'detail':'detail none'">
                    <div class="new flex-center-S" style="margin-bottom:10px">
                        <div v-if="item.noticeNew" class="addBtn">新增</div>
                        <div class="addContent"><pre>{{item.noticeNew}}</pre></div>
                    </div>
                    <div class="fix flex-center-S">
                        <div v-if="item.noticeRepair" class="fixBtn">修复</div>
                        <div class="fixContent"><pre>{{item.noticeRepair}}</pre></div>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    mounted(){
        ...
        this.activeTab=this.selectData.type || 'WEB'
